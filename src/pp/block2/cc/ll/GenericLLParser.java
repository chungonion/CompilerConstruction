...
public class GenericLLParser implements Parser {
	...
	private Map<NonTerm, List<Rule>> calcLL1Table() {
		Map<NonTerm, List<Rule>> map = new HashMap<NonTerm, List<Rule>>();
		for (NonTerm nt : g.getNonterminals()) {
			...
			for (Rule r : g.getRules(nt)) {
				for (Term t : calc.getFirstp().get(r)) {
					newRules.set(t.getTokenType(), r);
				}

				if (calc.getFirstp().get(r).contains(Symbol.EOF)) {
					newRules.set(Symbol.EOF.getTokenType(), r);
				}
			}
			map.put(nt, newRules);
		}
